...
import { HostEnvironmentDefault } from './hostEnv.mjs';
import { findOwnerCsProjectFile, getCurrentScriptDir, safeReadFile } from './cliUtils.mjs';

import type { GeneratorInitialization } from '../src/types.mjs';
import { LhqModel } from '../src/api/schemas.mjs';
import { GeneratedFile } from '../src/api/types.mjs';
import { isNullOrEmpty } from '../src/utils.mjs';
import { Generator, GeneratorHostDataKeys } from '../src/generator.mjs';
import { getRootNamespaceFromCsProj } from '../src/generatorUtils.mjs';
import { Duration } from '../src/duration.mjs';
...
export async function generateFromLhq(lhqFileName: string, csProjectFileName: string, outputDir: string,
    ...
    const hbsTemplatesDir = path.join(getCurrentScriptDir(), '..', 'hbs');
    ...
    if (isNullOrEmpty(rootNamespace)) {
        ...
        if (isNullOrEmpty(rootNamespace)) {
            ...
            rootNamespace = getRootNamespaceFromCsProj(path.basename(lhqFileName), t4FileName, csProjectFileName, csProjectFileContent);
        }
