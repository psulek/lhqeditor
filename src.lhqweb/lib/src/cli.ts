...
import {
    Duration, GeneratedFile, Generator, GeneratorHostDataKeys, GeneratorInitialization,
    type LhqModel, generatorUtils, isNullOrEmpty, jsonParseOrDefault, tryRemoveBOM,
    HostEnvironment,
    valueOrDefault
} from './index';
import { CSharpNamespaceInfo } from './generatorUtils';
...
async function validateLhqModelFile(lhqFileName: string): Promise<void> {
...
async function generateFromLhq(hbsTemplatesDir: string, lhqFileName: string, csProjectFileName: string, outputDir: string,
    ...
    - c# project file: ${hasCsProj ? csProjectFileName : '-'} (${csProjFound ? 'auto found' : hasCsProj ? 'cmd' : ''})
    - out dir: ${outputDir}\n`);
...
(async () => {
    try {
        ...
        const cliToolNameColor = pc.blueBright(cliToolName);
        ...
        program
            ...
            .addHelpText(
                ...
                `
Examples:
  ${cliToolNameColor} file.lhq ${pc.cyan('generate')} ${pc.yellow('Strings.lhq')} --project MyProject.csproj --out ./output --data namespace=Namespace1
  ${cliToolNameColor} file.lhq ${pc.yellow('Strings.lhq')} -o ./output
  ${cliToolNameColor} file.lhq ${pc.cyan('validate')} ${pc.yellow('Strings.lhq')}

  * when [command] is omitted, the default command is ${pc.cyan('generate')}.
`);
        ...
        const generateCommand = new Command('generate')
            ...
            .action(async (options) => {
                ...
                await generateFromLhq(hbsTemplatesDir, lhqfile, options.project ?? '', options.out ?? '.', hostData);
            });
